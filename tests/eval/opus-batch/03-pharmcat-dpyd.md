...
Find the installed PharmCAT DPYD *1/*1 diplotype row. Return its exact diplotype, diplotypekey, generesult, lookupkey, and phenotype, with stable row ID, member, and snapshot identity. If the source contains multiple matching rows, disclose the ambiguity rather than choosing silently.
